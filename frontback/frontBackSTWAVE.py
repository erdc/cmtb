...
def STsimSetup(startTime, inputDict,allWind , allWL, allWave, loc_dict=None):
    ...
    wavepacket = prepdata.prep_spec(rawspec, version_prefix, datestr=dateString, plot=plotFlag, full=full, outputPath=path_prefix)
...
def STanalyze(startTime, inputDict):
    ...
    for gg, station in enumerate(NestedStations):
        if station != ':': # stations marked with ':' for file names are not in the nested simulation
            coords = gp.FRFcoord(stat_packet['Easting'][0, gg], stat_packet['Northing'][0, gg]) # getting lat lon
            stat_dataNest = {
                'time': nc.date2num(modelpacket_nest['time'][:], units='seconds since 1970-01-01 00:00:00'),
